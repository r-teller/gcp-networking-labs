...
resource "google_compute_instance" "core_wan_to_distro_lan_vyos_usc1" {
  ...
  network_interface {
    network_ip = cidrhost(google_compute_subnetwork.core_wan["us-central1"].ip_cidr_range, (count.index + 3))
    subnetwork = google_compute_subnetwork.core_wan["us-central1"].self_link
  }

  network_interface {
    network_ip = cidrhost(google_compute_subnetwork.distro_lan["us-central1"].ip_cidr_range, (count.index + 3))
    subnetwork = google_compute_subnetwork.distro_lan["us-central1"].self_link
  }
...
resource "google_compute_instance" "distro_lan_to_access_truested_vyos_usc1" {
  ...
  network_interface {
    network_ip = cidrhost(google_compute_subnetwork.distro_lan["us-central1"].ip_cidr_range, (count.index + 13))
    subnetwork = google_compute_subnetwork.distro_lan["us-central1"].self_link
  }

  network_interface {
    network_ip = cidrhost(google_compute_subnetwork.access_trusted_0000["us-central1"].ip_cidr_range, (count.index + 3))
    subnetwork = google_compute_subnetwork.access_trusted_0000["us-central1"].self_link
  }

  network_interface {
    network_ip = cidrhost(google_compute_subnetwork.access_trusted_0001["us-central1"].ip_cidr_range, (count.index + 3))
    subnetwork = google_compute_subnetwork.access_trusted_0001["us-central1"].self_link
  }

  network_interface {
    network_ip = cidrhost(google_compute_subnetwork.access_trusted_0002["us-central1"].ip_cidr_range, (count.index + 3))
    subnetwork = google_compute_subnetwork.access_trusted_0002["us-central1"].self_link
  }
...
resource "google_network_connectivity_spoke" "access_trusted_0000_appliance_usc1" {
  project = var.project_id
...
resource "google_compute_router_peer" "access_trusted_0000_appliance_usc1_nic0" {
  for_each = { for idx, instance in google_compute_instance.distro_lan_to_access_truested_vyos_usc1 : idx => instance }

  project = var.project_id

  name      = format("%s-%s", each.value.name, "nic0")
  router    = google_compute_router.access_trusted_0000["us-central1"].name
  region    = google_compute_router.access_trusted_0000["us-central1"].region
  interface = google_compute_router_interface.access_trusted_0000_appliance_nic0["us-central1"].name
  ...
  depends_on = [
    google_network_connectivity_spoke.access_trusted_0000_appliance_usc1
  ]
}

resource "google_compute_router_peer" "access_trusted_0000_appliance_usc1_nic1" {
  for_each = { for idx, instance in google_compute_instance.distro_lan_to_access_truested_vyos_usc1 : idx => instance }
  ...
  router    = google_compute_router.access_trusted_0000["us-central1"].name
  region    = google_compute_router.access_trusted_0000["us-central1"].region
  interface = google_compute_router_interface.access_trusted_0000_appliance_nic1["us-central1"].name
