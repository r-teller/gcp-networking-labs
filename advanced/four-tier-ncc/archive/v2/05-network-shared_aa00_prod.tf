...
resource "google_compute_router_nat" "shared_aa00_prod" {
  for_each = toset(distinct([for subnetwork in local._networks["shared_aa00_prod"].subnetworks : subnetwork.region if(try(local._networks["shared_aa00_prod"].cloud_nat_all_subnets, false) || try(contains(subnetwork.tags, "cloud_nat"), false))]))
