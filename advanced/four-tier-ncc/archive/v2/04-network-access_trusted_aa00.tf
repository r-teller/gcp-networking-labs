...
resource "google_compute_router" "access_trusted_aa00" {
  ...
  bgp {
    ...
    asn               = try(local._networks["access_trusted_aa00"].regional_asn[each.key], local._networks["access_trusted_aa00"].shared_asn, local._default_asn)
...
resource "google_compute_router_nat" "access_trusted_aa00" {
  for_each = toset(distinct([for subnetwork in local._networks["access_trusted_aa00"].subnetworks : subnetwork.region if(try(local._networks["access_trusted_aa00"].cloud_nat_all_subnets, false) || try(contains(subnetwork.tags, "cloud_nat"),false))]))
...
resource "google_compute_router_interface" "access_trusted_aa00-appliance-nic0" {
...
resource "google_compute_router_interface" "access_trusted_aa00-appliance-nic1" {
  for_each = { for x in local._networks.access_trusted_aa00.subnetworks : x.ip_cidr_range => x if contains(try(x.tags, []), "network_appliance") }
  ...
  router              = google_compute_router.access_trusted_aa00[each.value.region].name
  region              = each.value.region
  redundant_interface = google_compute_router_interface.access_trusted_aa00-appliance-nic0[each.key].name
