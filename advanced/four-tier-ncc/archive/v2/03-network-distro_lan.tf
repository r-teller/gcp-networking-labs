...
resource "google_compute_router_nat" "distro_lan" {
  for_each = toset(distinct([for subnetwork in local._networks["distro_lan"].subnetworks : subnetwork.region if(try(local._networks["distro_lan"].cloud_nat_all_subnets, false) || try(contains(subnetwork.tags, "cloud_nat"),false))]))
