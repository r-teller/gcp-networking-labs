...
resource "google_compute_router_nat" "core_wan" {
  for_each = toset(distinct([for subnetwork in local._networks["core_wan"].subnetworks : subnetwork.region if(try(local._networks["core_wan"].cloud_nat_all_subnets, false) || try(contains(subnetwork.tags, "cloud_nat"),false))]))
